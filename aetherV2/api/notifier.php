...
HTML;
    ...
    private static function sendMail(string $to, string $subject, string $html): bool {
        ...
        $headers  = "MIME-Version: 1.0\r\n"
                  ...
                  . "X-Mailer: Aether/2.0";
        try {
            // 1) Try raw SMTP if host configured
            if (AETHER_SMTP_HOST) {
                return self::smtpSend($to, $subject, $html, $headers);
            }
    ...
    private static function smtpSend(string $to, string $subject, string $html, string $extraHeaders): bool {
        $host = AETHER_SMTP_HOST;
        $port = (int)AETHER_SMTP_PORT;
        $user = AETHER_SMTP_USERNAME;
        $pass = AETHER_SMTP_PASSWORD;
        $from = AETHER_SMTP_FROM_EMAIL ?: $user;
        $tls  = $port === 465;
